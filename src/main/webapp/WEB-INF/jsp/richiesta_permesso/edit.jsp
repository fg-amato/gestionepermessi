					...
					<h6 class="card-title">I campi con <span class="text-danger">*</span> sono obbligatori</h6>
						<form:form modelAttribute="edit_richiesta_attr" method="post" action="${pageContext.request.contextPath}/richieste_permesso/update" novalidate="novalidate" class="row g-3">
						  <input type="hidden" name="id" value="${edit_richiesta_attr.id }">
								    ...
								    <spring:bind path="tipoPermesso">
									    <select class="form-select ${status.error ? 'is-invalid' : ''}" id="tipoPermesso" name="tipoPermesso" required>
									    	<option value="" selected> - Selezionare - </option>
									      	<option value="FERIE" ${edit_richiesta_attr.tipoPermesso == 'FERIE'?'selected':''} >FERIE</option>
									      	<option value="MALATTIA" ${edit_richiesta_attr.tipoPermesso == 'MALATTIA'?'selected':''} >MALATTIA</option>
									    </select>
								    </spring:bind>
							...
							<div class="col-md-6  ${edit_richiesta_attr.tipoPermesso == 'MALATTIA'? '':'d-none'}" id ="prova">
									...
									<input type="text" class="form-control ${status.error ? 'is-invalid' : ''}" name="codiceCertificato" id="codiceCertificato" placeholder="Inserire il codice certificato" value="${edit_richiesta_attr.codiceCertificato }">
									</spring:bind>
							...
							<div class="col-md-6 datiMalattia ${edit_richiesta_attr.tipoPermesso == 'MALATTIA'? '':'d-none'}" id="datiMalattia">
